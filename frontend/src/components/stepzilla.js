...
export default function StepZilla(props) {
    ...
    const renderSteps = () => {
        return props.steps.map((s, i) => ( <
            li className = { getClassName('progtrckr', i) }
            onClick = {
                (evt) => { jumpToStep(evt); }
            }
            key = { i }
    ...
    return ( <
        ...
        onKeyDown = {
            ...
            props.showSteps ?
            <
            ol className = "progtrckr" > { renderSteps() } <
            /ol> : <
            span > < /span>
        }

        { compToRender } <
        div style = { props.showNavigation ? {} : hidden }
        ...
        id = "prev-button" > { props.backButtonText } <
        /button> <
        button type = "button"
